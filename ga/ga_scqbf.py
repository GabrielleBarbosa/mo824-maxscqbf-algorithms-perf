...
class GA_SCQBF(AbstractGA):
    ...
    def __init__(self, popSize: int, mutationRate: float, filename: str,
        ...
        if targetValue:
            self.bestKnown = targetValue
        else:
            self.bestKnown = float("inf")
    ...
    def solve(self):
        ...
        self.time_best_sol = (int(time.time() * 1000) - self.startTime) / 1000.0
        self.iter_best_sol = 0
        ...
        while (int(time.time() * 1000) - self.startTime) < self.timeLimit and self.bestSol.cost < self.bestKnown:
            ...
            generation += 1
        self.total_iterations = generation
        return self.bestSol, self.total_iterations, self.time_best_sol, self.iter_best_sol
